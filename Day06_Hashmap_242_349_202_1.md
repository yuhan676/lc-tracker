...
class Solution {
    public boolean isAnagram(String s, String t) {
        ...
        //the underlying mechanism of .euqals()
        //is comparing all the key-value pairs in the entryset 
        //so time complexity is O(n), space complexity is O(1) (no new space created)
        //易错点： mapS==mapT比较的是memory address
...
* 二刷： .stream().mapToInt()拆箱有两种方式
```java
 int[] res = resSet.stream().mapToInt(Integer::intValue).toArray();
 int[] res = resSet.stream().mapToInt(x->x).toArray();
```
<img width="802" height="249" alt="Screenshot 2025-11-17 at 23 29 04" src="https://github.com/user-attachments/assets/fc062bbc-0a46-4e39-8879-cc71afe57b79" />
...
* 二刷笔记：完全没有必要用1002， 1001个就够用（0-1000只有1001个数字）
```java
class Solution {
    public int[] intersection(int[] nums1, int[] nums2) {
...
class Solution {
    public boolean isHappy(int n) {
        ...
        //二刷难点：这里为什么是&&?
        //因为只有当它同时满足!=1, 之前没见过，才继续loop
        //假如之前见过，不等于1， 那必须停止loop
        //假如等于1， 之前没见过，也必须停止
        //如果用||, 以上两种情况都会继续while loop而不是跳出来，所以不能用||
