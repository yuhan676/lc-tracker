...
class Solution {
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        Arrays.sort(nums);
        for (int i =0; i< nums.length;i++){
            //这里其实不用管正数负数的剪枝,只要i和j检查最小可能的和，和最大可能的和就行
            if (i > 0 && nums[i] == nums[i-1]) continue;
            for (int j = i+1;j<nums.length;j++){
                if ( j > i+1 && nums[j] == nums[j-1]) continue;
                int l = j+1, r = nums.length-1;
                    while (l < r){
                        //这里用long，防止int栈溢出， int最大是2^32, 2,147,483,647
...
```
## 二刷新题： 706 Design HashMap
* https://leetcode.com/problems/design-hashmap/description/
* youtube视频： https://www.youtube.com/watch?v=GLDyUKaQoYg
### 解法1
* 0 < key, value <- 10^6
* 所以我们可以用一个size = 10^6 + 1的数组
* 时间复杂度（O(1)), 空间复杂度： 10^6
* 但是这样就不是hashmap，没有用上hashcode了
```java
class MyHashMap {
    ...
    public MyHashMap() {
        // choose a prime bucket size ~ 10^4
		//怎么决定什么bucketsize才是最合适的？
        //我们想要reduce clustering in modulo hashing, 最好选择prime number, 这样，每个bucket list 可以尽量一样长
        //情况一：假如我们选择size == root(key range) = 10^3, 那最长的list还是10^3那么长，不够好
        //情况2:假如我们选择size = key range / 10 = 100k, 那performance很好，占空间也ok100k * 8 bytes (reference) = 800 KB， logically more like ~1MB after alignment
        //情况3:直接用size = key range, 可以避免hash collision,但占空间特别大 （10mb）
        //chat推荐：适用10^5附近的prime number e.g. 10007，这样可以避免clustering，也让performance和memory space都比较理想
        this.size = 10007;
