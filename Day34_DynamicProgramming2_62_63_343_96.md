...
* 看了视频但是没有完全理解，二刷重新尝试
* 二刷：
* - 这题说实话我感觉代码随想录讲得不够好，重要的不是数字n和给出来的例子，而在于数字n能组成的bst数量 = 头节点为1的bst数量 + 头节点为2的bst的数量 + 头节点为3的bst的数量 ……+头节点为n的bst的数量。
- 也就是说int j = 1, j≤n; j++来遍历所有头节点的可能性
- dp[n] 的意思：有n个数字，有多少种bst的排布方法
- 那上面这一系列数字，单个‘头节点为j的bst数量’为多少呢？就看它左子树有多少种可能排布，右子树有多少可能的排布，然后这两个数相乘，就是所有可能的排布。左子树可能的排布 = 有j-1个节点数字，有多少种排布 = dp[j-1]; 右子树可能的排布：有 dp[n-j] 种可能的排布（因为右边一共有n-j个数字）
- 也就是说，用i从从1遍历到n，都要构造dp[i]
- 每一个dp[i]怎么计算？计算它的子排布方式，就得用j从1遍历到i，看看用不同的j当作头节点，一共有多少种排布方式，把他们的乘积累加
- 时间复杂度：O(n^2), 外层最多跑n次，里层最多跑n次
```java
class Solution {
