...
* 二刷笔记：层序遍历时间和空间复杂度都是O(n), 而递归的话时间依然是O(n)因为需要遍历完所有的节点，但是空间复杂度是O（h）（最坏的时候O(n)当树退化成链表，最好的时候O(log n )）， 因为不需要记录所有节点，唯一空间就是递归栈。此处h是树的高度
```java
class Solution {
...
class Solution {
    ...
    private boolean dfs (TreeNode root, int targetSum, int currSum){
        ...
        //这个地方不能直接在bfs left, right以后return false
        //left, right只要有一个是true，就反回true
...
```
# 二刷笔记
* 空间复杂度上，辅助空间用递归 + path 最大深度= O(h), 输出空间最坏为O(n^2)，假设每一条路径都符合条件
* 时间复杂度上因为咱们要便利每个节点，所以O(n)就好，n为总节点个数
```java
class Solution {
    public List<List<Integer>> pathSum(TreeNode root, int targetSum) {
    ...
    private void dfs(TreeNode node, int targetSum, int currSum, List<Integer> path, List<List<Integer>> res){
        ...
        if (node.left == null && node.right ==null){
            if (currSum == targetSum){
                //这里记得path就算add以后也会不停变化，随着回溯被删除，所以加一个copy最好
